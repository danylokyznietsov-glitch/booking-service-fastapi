...
@router.post("")
async def create_hotel(hotel: SHostel, session: AsyncSession = Depends(get_async_session)):
...
@router.put("/{hotel_id}")
async def update_hotel(hotel_id: int, hotel_data: SHostel, session: AsyncSession = Depends(get_async_session)):
...
@router.get("/")
def read_root():
    return {"message": "Привіт, це мій Букінг!"}
